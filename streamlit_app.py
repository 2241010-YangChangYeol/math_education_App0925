...
import re
if re.search(r"x", func_str):
    x_val = st.number_input("x값을 입력하세요 (그래프에는 반영되지 않음)", value=1.0, key="x_value")
else:
    x_val = None
...
if st.button("적분 그래프 그리기"):
    ...
    st.success(f"적분값: {integral_val:.4f}")
        ...
        # x값이 입력된 경우 해당 값에서의 함수값도 표시
        if x_val is not None:
            try:
                y_val = f_lamb(x_val)
                st.info(f"입력한 x={x_val}에서의 함수값: {y_val}")
            except Exception as e:
                st.warning(f"x값 계산 오류: {e}")
    except Exception as e:
